...
FACE_CUBE_PERMUTATIONS = {
    'U': Permutation.cyclic([ 0, 1, 3, 2], 8).square(),
    'R': Permutation.cyclic([ 3, 1, 5, 7], 8).square(),
    'F': Permutation.cyclic([ 2, 3, 7, 6], 8).square(),
    'D': Permutation.cyclic([ 6, 7, 5, 4], 8).square(),
    'L': Permutation.cyclic([ 0, 2, 6, 4], 8).square(),
    'B': Permutation.cyclic([ 1, 0, 4, 5], 8).square()
}
...
class RubikCubeModel():
    ...
    class Cube:
        def __init__(self, showbase):
            ...
            configure = [
                [Vec3( 0.0,  0.0,  1.0), Vec3(0,   0,   0)], # U
                [Vec3( 1.0,  0.0,  0.0), Vec3(0,   0,  90)], # R
                [Vec3( 0.0,  0.0,  0.0), Vec3(0,  90,   0)], # F
                [Vec3( 0.0,  0.0,  0.0), Vec3(0,   0, 180)], # D
                [Vec3( 0.0,  0.0,  0.0), Vec3(0,   0, -90)], # L
                [Vec3( 0.0,  0.1,  0.0), Vec3(0, -90,   0)]] # B
    ...
    DEFAULT_COLORS = [FACE_COLOR_INITIAL[c] for c in list("WWWWBBBBRRRRYYYYGGGGOOOO")]
    def set_color(self, colors = None):
        if colors is None: colors = self.DEFAULT_COLORS
        for face, n in zip(list("URFDLB"), range(0, 6)):
            for i, j in zip(FACE_CUBE_INDICES[face], range(0, 4)):
                self.cubes[i].set_face_color(face, colors[4 * n + j])
...
class RubikCubeSolverGUI(ShowBase):
    ...
    def start_solving(self):
        ...
        try:
            ...
            solution_str = "R F L R D B F"
            """
            # kociemba library への入力形式に変換する
            table = {}
            for f, n in zip(list("URFDLB"), range(6)): table[c[4 * n + 2]] = f
...
class InputPanels(DirectFrame):
    ...
    FACE_UPPER_LEFT = [[0, 2, 'U'], [2, 4, 'R'], [2, 2, 'F'], [4, 2, 'D'], [2, 0, 'L'], [2, 6, 'B']]
    ...
    def __init__(self, parent=None, **kw):
        ...
        self.panel_holder = [[None for _ in range(8)] for _ in range(6)]
        for i, j in product(range(6), range(8)):
        ...
        self.panels = []
        for c in self.FACE_UPPER_LEFT:
            p = range(c[0], c[0] + 2); q = range(c[1], c[1] + 2)
